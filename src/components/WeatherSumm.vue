...
const lat = computed( () => props.weatherInfo?.coord?.lat)
...
function url(lat, lon) {
   return `https://www.windy.com/articles/weather-phenomena-what-s-the-difference-between-sustained-winds-and-wind-gusts-10390?satellite,${lat},${lon},9`;
}
...
<template>
     <div v-if="weatherInfo?.weather" class="summary">
         ...
         <div class="text-block">
            <a 
              :href="url(weatherInfo?.coord?.lat, weatherInfo?.coord?.lon )" target="_blank"
              >More information
            </a>
         </div>
      </div>
